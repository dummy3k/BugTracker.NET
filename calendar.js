...
Modified to work well with BugTracker.NET.  Specifically,
calls "on_change()" method when date value changes - corey trager 12/02
...
Calendar.prototype.show = function() {
	...
	ggWinContent += ("[<A style='font-size: 8pt' HREF=\"javascript:void(0);\" " +
	...
	for (j=vFirstDay; j<7; j++) {
		vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT FACE='" + fontface + "'>" + 
			"<A HREF='javascript:void(0);' " + 
				...
				"onMouseOut=\"window.status=' '; return true;\" " +
// Corey Trager Jul 2005 - changed
// to allow form fields whose names have spaces in them
// "onClick=\"document." + this.gReturnItem + ".value='" + 
				"onClick=\"document.getElementById('" 
				+ (this.gReturnItem.substr(this.gReturnItem.indexOf(".")+1)) 
				+ "').value='" + 
				this.format_data(vDay) + 
				"'; try{on_change()} catch(e){} ggPosX=-1;ggPosY=-1;nd();nd();\">" + 
...
// "onClick=\"document." + this.gReturnItem + ".value='" + 
				...
				+ "').value='" + 
					this.format_data(vDay) + 
					"'; try{on_change()} catch(e){} window.scroll(0,ggPosY);ggPosX=-1;ggPosY=-1;nd();nd();\">" + 
				this.format_day(vDay) + 
				"</A>" + 
				"</FONT></TD>";
			vDay=vDay + 1;
...
// corey - changed the date format specifiers to match C#
